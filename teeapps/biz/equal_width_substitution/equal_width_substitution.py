...
def run_equal_width_substitution(task_config: dict):
    logging.info("Running equal width substitution...")
    ...
    bin_num = rules["bin_num"]
    global_min_max = rules["global_min_max"]

    bin_edges = {}
    for k, v in global_min_max.items():
        global_min, global_max = v
        bin_edges[k] = np.linspace(global_min, global_max, bin_num + 1)
    ...
    for i in range(len(output_schema.features)):
        if output_schema.features[i] in global_min_max:
...
def main():
    ...
    with open(task_config_path, "r") as task_config_f:
        ...
        run_equal_width_substitution(task_config)
